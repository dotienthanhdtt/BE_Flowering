...
export class MergeUserAiScenariosIntoScenarios1779800000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    // 1. Enum changes. RENAME VALUE and ADD VALUE both commit immediately and
    //    are usable in the same transaction in Postgres 12+.
    ...
    // 3. Backfill personal rows from user_ai_scenarios. Preserve UUIDs so
    ...
    // 4. CHECK constraint enforcing the type/owner_id/category_id contract.
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    // Rename enum value back. Cannot drop the 'personal' value safely in
    // Postgres without rebuilding the type — leave it as harmless orphan.
